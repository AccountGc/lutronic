...
public class StandardGroupwareService extends StandardManager implements GroupwareService {
	...
	@Override
	public Map<String,Object> listNoticeAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    while(qr.hasMoreElements()) {
			...
			xmlBuf.append("<cell><![CDATA[<a href=javascript:gotoView('" + data.oid + "')>" + data.title + "</a>]]></cell>");
	...
	@Override
	public Map<String,Object> listWorkItemAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		while(qr.hasMoreElements()){
			...
			/*if(result!=null){
				System.out.println("StandardGroupwareService ::: result = " + result.size());
			}else{
			...
			if (result.hasMoreElements()) {
				...
				String processState = wfprocess.getState().getFullDisplay();
...
//			xmlBuf.append("<cell><![CDATA[]]></cell>");
			...
			//xmlBuf.append("<cell><![CDATA[<a href=javascript:gotoView('" + item.getTaskURLPathInfo() + "&order=" + order + "');>" + activity.getName() + "</a>]]></cell>");
			xmlBuf.append("<cell><![CDATA[<a href=javascript:gotoView('" + item.getTaskURLPathInfo() + "','" + oid + "','" + viewOid + "');>" + activity.getName() + "</a>]]></cell>");
			if(isDistribute){
				xmlBuf.append("<cell><![CDATA[<a href=javascript:openDistributeView('" + viewOid + "');>" + objName1 + "["+objName2+"]" + "</a>]]></cell>");
			}else{
				xmlBuf.append("<cell><![CDATA[<a href=javascript:openView('" + viewOid + "');>" + objName1 + "["+objName2+"]" + "</a>]]></cell>");
			}
			...
			if("수신".equals(activity.getName())) {
				...
				xmlBuf.append("<cell><![CDATA[" + activity.getParentProcess().getCreator().getFullName() + "]]></cell>");
			}
			
			if(wfProcessOid==null || wfProcessOid.equals("") || wfProcessOid=="" ){
				xmlBuf.append("<cell><![CDATA[" + pbo.getLifeCycleState().getDisplay(Message.getLocale()) + "]]></cell>");
			}else {
				if(isAdmin) {
					xmlBuf.append("<cell><![CDATA[<a href='javascript:void(0);' onclick=javascript:processHistory('/Windchill/ptc1/process/info?oid=" + wfProcessOid + "');>" + pbo.getLifeCycleState().getDisplay(Message.getLocale()) + "</a>]]></cell>");
				}else {
					xmlBuf.append("<cell><![CDATA[" + pbo.getLifeCycleState().getDisplay(Message.getLocale()) + "]]></cell>");
				}
			}
			xmlBuf.append("<cell><![CDATA[" + DateUtil.getDateString(item.getPersistInfo().getCreateStamp(), "d") + "]]></cell>");
	...
	public ModelAndView processInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(activity!=null) {
			...
			if( deadline != null )
			{
				if(deadline.after(new java.util.Date())) {
					model.addObject("deadline", "<b><font color='green'>"+(""+deadline).substring(0, 10)+"</font></b>");
				} else {
				    isDelay = true;
				    model.addObject("deadline", "<b><font color='red'>"+(""+deadline).substring(0, 10)+"</font></b>");
				}
			}
		}
		...
		QueryResult result = wt.workflow.engine.WfEngineHelper.service.getAssociatedProcesses(per, null, WCUtil.getWTContainerRef());
	...
	public ModelAndView approval(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(wfItem != null) {
			...
			for(WFItemUserLink wfLink : vec) {
				...
				if(wfLink.getProcessOrder()==0)state="기안";
	...
	public List<Map<String,Object>> getApprovalList(String oid) throws Exception {
		...
		if (wfItem != null) {
			...
			for (WFItemUserLink wfLink : vec) {
				...
				//System.out.println(wfLink.getUser().getFullName()+"\t"+state+"\tlink Oid = "+CommonUtil.getOIDString(wfLink)+ "\tisCommentEdit="+isCommentEdit+"\tisAuthComment="+isAuthComment);
				list.add(map);
			}
		}
	...
	public void approveAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if ("reassign".equals(cmd)) { // 위임
			...
			try {
				...
				boolean isECO = false;
				//System.out.println("[ActionApprove][wtobject instanceof EChangeActivity]:" + (wtobject instanceof EChangeActivity));
				if (wtobject instanceof EChangeActivity) {
					...
					if (wfItem == null) {
						wfItem = WFItemHelper.service.newWFItem(wtobject, workItem.getOwnership().getOwner().getPrincipal());
					}
				}
				
				if (wfItem != null) {
					//System.out.println("[ActionApprove][wfItem]:" + wfItem.getPersistInfo().getObjectIdentifier().toString());
				}
	...
	@Override
	public Map<String,Object> listItemAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		while(qr.hasMoreElements()) {
	       ...
	       // }
			...
			xmlBuf.append("<cell><![CDATA[" + DateUtil.getDateString( PersistenceHelper.getCreateStamp(wfItem), "d" ) + "]]></cell>");
			...
			WfState wfState = null;
			String wfProcessOid = "";
			QueryResult wqr = wt.workflow.engine.WfEngineHelper.service.getAssociatedProcesses(per, null, WCUtil.getWTContainerRef());
			if (wqr.hasMoreElements()) {
				wfprocess = (WfProcess)wqr.nextElement();
				wfProcessOid = wfprocess.getPersistInfo().getObjectIdentifier().getStringValue();
				wfState = wfprocess.getState();
				if(CommonUtil.isAdmin()) {
					xmlBuf.append("<cell><![CDATA[<a href=javascript:processHistory('/Windchill/ptc1/process/info?oid=" + wfProcessOid +"');>" + rState.getDisplay(Message.getLocale()) + "</a>]]></cell>");
				}else {
	...
	@Override
	public List<Map<String,String>> loadApprovalLine(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(wfItem!=null)
		{
			...
			for(int i = 0 ; i < vec.size() ; i++)
			{
				...
				if(link.getActivityName().equals("기안")) continue; 
				...
				String userInfo = wtuserOid+","+people.getPersistInfo().getObjectIdentifier().toString()+","+deptOid+","+linkUser.getName()+","+fullName+","+deptName+","+duty+","+dutycode+","+email+","+fullName;
	...
	@Override
	public Map<String,Object> wfProcessInfoAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if("".equals(command)) {
			...
			if((object == null)) {
				...
				QueryResult qr = WfEngineHelper.service.getAssociatedProcesses((Persistable)object, null, WCUtil.getWTContainerRef());
				...
				if(obj != null) {
					...
					if(obj instanceof EPMDocument) {
						...
						if(attrName.equals("SeqCheck")){
							...
							IBAUtil.changeIBAValue(epm, AttributeKey.EPMKey.IBA_SEQCHECK, "Y", "boolean");
						}else{
							IBAUtil.changeIBAValue(epm, attrName, attrValue, "string");
						}
						
						msg = Message.get("변경이 완료되었습니다.");
						result = "S";
						...
						if(attrName.equals("SeqCheck")){
							...
							IBAUtil.changeIBAValue(part, attrName, attrValue, "string");
						}
						msg = Message.get("변경이 완료되었습니다.");
						result = "S";
					}else {
						msg = Message.get("대상 객체가 아닙니다.");
						result = "F";
					}
					*/
					data.setResult(true);
	...
	public Map<String,String> multiPublishingAction(HttpServletRequest request, HttpServletResponse response) {
		...
		if(predate.length() > 0) {
			try{
				...
				msg = Message.get("변환요청에 성공했습니다.");
				result = "S";
			}catch(Exception e) {
				e.printStackTrace();
				msg = Message.get("변환요청이 실패되었습니다.");
				result = "F";
			}
		}
