...
public class StandardECAService extends StandardManager implements ECAService {
	...
	public String deleteRootDefinitionAction(HttpServletRequest req) throws Exception {
		...
		try {
			...
			List<EChangeActivityDefinition> list = getActiveDefinition(
	...
	public List<EChangeActivityDefinition> getActiveDefinition(long rootOid) throws Exception {
		...
		qs.appendAnd();
		qs.appendWhere(new SearchCondition(EChangeActivityDefinition.class, "rootReference.key.id", "=", rootOid),
				new int[] { idx1 });
	...
	@Override
	public Map<String, Object> listActiveDefinitionAction(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		for (EChangeActivityDefinition def : list) {
			...
			xmlBuf.append("<cell><![CDATA[<a href=javascript:updateActivityDefinition('" + oid + "')>" + def.getName()
					+ "</a>]]></cell>");
		...
		if (steps != null) {
			...
			for (int i = 0; i < steps.length; i++) {
				...
				if (!ecaMap.containsKey(ecaOid)) {
					...
					if (eca.getLifeCycleState().toString().equals("COMPLETED"))
						continue;
					isActivity = true;
					...
					query.appendSelect(new ClassAttribute(target, "thePersistInfo.theObjectIdentifier.classname"),
							new int[] { idx }, false);
					query.appendSelect(new ClassAttribute(target, "thePersistInfo.theObjectIdentifier.id"),
							new int[] { idx }, false);
					query.appendSelect(new ClassAttribute(target, "primaryBusinessObject.key.classname"),
							new int[] { idx }, false);
					...
					query.appendSelect(new ClassAttribute(target, "thePersistInfo.createStamp"), new int[] { idx },
							false);
					...
					if (query.getConditionCount() > 0)
						query.appendAnd();
					query.appendWhere(new SearchCondition(target, "primaryBusinessObject.key.classname",
							SearchCondition.LIKE, "%" + ecaOid + "%"), new int[] { idx });
					...
					if (null != workitem) {
						...
						if (wfitem != null) {
							...
							WFItemUserLink link = WFItemHelper.service.getOwnerApplineLink(oldUser, wfitem,
									activityName);
							...
							if (link != null) {
								...
								// 기존객체 수정
								link.setState("위임");
								String comment = "'" + oldUser.getFullName() + "'로 부터 '" + user.getFullName()
										+ "'으로 업무 위임";
				...
				// 접수중 변경 프로세스 종료
				// LifeCycleHelper.service.setLifeCycleState((LifeCycleManaged)eca,
				// State.toState("INTAKE"));
	...
	public List<EChangeActivity> getECAList(ECOChange eo) throws Exception {
		...
		qs.appendAnd();
		qs.appendWhere(new SearchCondition(cls1, "eoReference.key.id", SearchCondition.EQUAL, longOid),
				new int[] { idx1 });

		qs.appendOrderBy(new OrderBy(new ClassAttribute(cls2, "sort"), false), new int[] { idx2 });
		qs.appendOrderBy(new OrderBy(new ClassAttribute(cls1, "sortNumber"), false), new int[] { idx1 });
//        System.out.println(qs);
	...
	public List<EChangeActivity> getNextActivity(EChangeActivity eca) throws Exception {
		...
		qs.appendWhere(new SearchCondition(EChangeActivity.class, "eoReference.key.id", "=",
				eca.getEo().getPersistInfo().getObjectIdentifier().getId()), new int[] { jj });
		qs.appendAnd();
		qs.appendWhere(new SearchCondition(EChangeActivity.class, "sortNumber", ">", eca.getSortNumber()),
				new int[] { jj });
		qs.appendAnd();
		qs.appendWhere(new SearchCondition(EChangeActivity.class, "step", "<>", eca.getStep()), new int[] { jj });

		qs.appendOrderBy(new OrderBy(new ClassAttribute(EChangeActivity.class, "sortNumber"), false), new int[] { jj });
...
//		System.out.println(qs.toString());
	...
	public boolean isStepComplete(EChangeActivity eca) throws Exception {
		...
		qs.appendAnd();
		qs.appendWhere(new SearchCondition(EChangeActivity.class, EChangeActivity.LIFE_CYCLE_STATE, "<>", "COMPLETED"),
				new int[] { jj });
	...
	public void createDocumentActivityLink(WTDocument doc, String parentOid) throws Exception {
		...
		DocumentActivityLink link = DocumentActivityLink.newDocumentActivityLink((WTDocumentMaster) doc.getMaster(),
				eca);
		PersistenceHelper.manager.save(link);
	}
...
//			System.out.println("docList =" + docList.size());
	...
	/**
	 * EO,ECO 완료시 자동 생성 EO,ECO 승인 완료후 추가 산출물 등록
	 */
	@Override
	...
	public String getECAMaxStep(ECOChange eo) throws Exception {
		...
		SQLFunction maxFunction = SQLFunction.newSQLFunction(SQLFunction.MAXIMUM,
				new ClassAttribute(EChangeActivity.class, EChangeActivity.STEP));
		...
		qs.appendWhere(new SearchCondition(EChangeActivity.class, "eoReference.key.id", "=",
				eo.getPersistInfo().getObjectIdentifier().getId()), new int[] { idx });
...
//		System.out.println(qs);
	...
	public int getECAMaxSortNumber(ECOChange eo) throws Exception {
		...
		SQLFunction maxFunction = SQLFunction.newSQLFunction(SQLFunction.MAXIMUM,
				new ClassAttribute(EChangeActivity.class, EChangeActivity.SORT_NUMBER));
...
//		System.out.println(qs);
	...
	public boolean getEOActivity(ECOChange eo, String activityName) throws Exception {
		...
		qs.appendWhere(new SearchCondition(EChangeActivity.class, EChangeActivity.NAME, "=", activityName, false),
				new int[] { idx });
...
//		System.out.println(qs.toString());
	...
	private void _workingUserChange(EChangeActivity eca, WTUser currentUser, WTUser newUser) throws Exception {
		/*
		 * WTProperties prover = WTProperties.getServerProperties(); String server =
		 * prover.getProperty("wt.cache.master.codebase");
		 * 
		 * if (server == null) { server = prover.getProperty("wt.server.codebase"); }
		 ...
		 * 
		 * 
		 * System.out.println(eca.getName()+ " Forground ressign = "+
		 * currentUser.getFullName() +","+newUser.getFullName() ); String assignRoleName
		 * = "ASSIGNEE"; //WorkItem workItem= getWorkItem(eca);
		 * //E3PSWorkflowHelper.service.reassignWorkItem(workItem, newUser); /*
		 * E3PSWorkflowHelper.service.deleteParticipant( (LifeCycleManaged) eca,
		 * assignRoleName, currentUser.getName());
		 * E3PSWorkflowHelper.service.addParticipant( (LifeCycleManaged) eca,
		 * assignRoleName, newUser.getName());
		 */
	}
...
//		System.out.println(eca.getName()+ "Background  ressign = "+ currentUser.getFullName() +","+newUser.getFullName() );
	...
	private WorkItem getWorkItem(EChangeActivity eca) {
		...
		try {
			...
			qs.appendWhere(new SearchCondition(cls, "primaryBusinessObject.key.classname", SearchCondition.EQUAL, oid),
					new int[] { idx });
			qs.appendAnd();
			qs.appendWhere(new SearchCondition(cls, WorkItem.STATUS, SearchCondition.EQUAL, "POTENTIAL"),
					new int[] { idx });
...
//		System.out.println("ECO  상태값 변경 1 stateChange : " + eo.getEoNumber() +":" + isInwork +":eo isAPPROVE_REQUEST = " + isAPPROVE_REQUEST);
		if (!isInwork) {
			if (!isAPPROVE_REQUEST) {
				LifeCycleHelper.service.setLifeCycleState((LifeCycleManaged) eo, State.toState("APPROVE_REQUEST"),
						true);
			} else {
...
//					System.out.println("APPROVE_REQUEST State Change 소스 주석");
					// LifeCycleHelper.service.setLifeCycleState((LifeCycleManaged)eo,
