...
public class StandardECOService extends StandardManager implements ECOService {
	...
	public ResultData updateEOAction(HttpServletRequest req)throws Exception{
       ...
       try {
	    	...
	    	List<WTPart> completeList = null;
	    	List<WTPart> changeList = null;
	    	if(isECO){
	    		model = (String)ecoMap.get("model");								//제품명
	    		completeList = (List<WTPart>)ecoMap.get("completeList"); 	//완제품
	    		changeList = (List<WTPart>)ecoMap.get("changeList");	  	//변경 대상
	    	}
			*/
	    	...
	    	if(!CommonUtil.isAdmin()){
	    		...
	    		/*
	    		if (isActivity) {
		    		LifeCycleHelper.service.setLifeCycleState((LifeCycleManaged)eco, State.toState("ACTIVITY"), true);
				}else{
					LifeCycleHelper.service.setLifeCycleState((LifeCycleManaged)eco, State.toState("APPROVE_REQUEST"), true);
				}
				*/
	    	}
	    	if(null!=sdeloids)
	...
	private Map<String, Object> getEODataCollection(String[] partOids) throws Exception{
		...
		//System.out.println("getEODataCollection completeList = " + completeList.size());
		map.put("model", model);
	...
	public void completeECO(EChangeOrder eco) {
       ...
       try{
    	    ...
    	    //BOM 반영
    	    ////System.out.println("0.[ChangeECOHelper]{completeECO} = " +eco.getEoNumber() +" BOM 반영");
    	    ...
    	    if(eoType.equals(ECOKey.ECO_CHANGE)){
    	    	...
    	    	//Part,CAD 상태 변경 
        	    //System.out.println("1.[ChangeECOHelper]{completeECO} = " +eco.getEoNumber() +" 부품,도면 상태 변경");
    	    	...
    	    	//완제품의 하위 구조 승인 처리
    	    	//System.out.println("1.[ChangeECOHelper]{completeECO} = " +eco.getEoNumber() +","+eoType +" 완제품의 하위 구조 승인 처리");
    	   ...
    	   //System.out.println("[ChangeECOHelper]{completeECO} ERROR = " + e.getLocalizedMessage());
    	   e.printStackTrace();
	...
	private void addPartAction(HttpServletRequest request) throws Exception {
		...
		///	trx.start();
			...
			for(int i = 0 ; isSelecctBom!=null && i <isSelecctBom.length; i++){
				isSelecctBomVec.add(isSelecctBom[i]);
				//System.out.println("isDisuse = " + isSelecctBom[i]);
			}
			*/
			for(int i=0; partOids!=null && i<partOids.length; i++){
			...
			for(int i=0; partOids!=null && i<partOids.length; i++){
				...
				if(updatePart.containsKey(part.getNumber())){
					...
					link.setVersion(version);
					/*
					if(isSelecctBomVec.contains(part.getNumber())){
						link.setBaseline(true);
					}else{
						link.setBaseline(false);
					}
					*/
					link.setBaseline(true);
					PersistenceHelper.manager.save(link);
	...
	public void completeECOTEST(EChangeOrder eco) {
	       ...
	       try{
	    	    ...
	    	    if(eoType.equals(ECOKey.ECO_CHANGE)){
	    	    	...
	    	    	//ERP 완제품의 전송
	    	    	////System.out.println("3.[ChangeECOHelper]{completeECO} = " +eco.getEoNumber() +","+eoType +" ERP 전송");
	    			//ERPHelper.service.sendERP(eco);
