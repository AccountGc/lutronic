...
//				boolean isNonCheckout = !(partData.isGENERIC() || partData.isINSTANCE());
				...
				if (epm != null){
					if(isNonCheckout){
						...
						if (WorkInProgressHelper.isCheckedOut(epm)) {
							epm = (EPMDocument) WorkInProgressHelper.service.checkin(epm, Message.get("PDM에서 체크인 되었습니다"));
						}
					}
	...
	public String createPackagePartAction(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
            ...
            for(Map<String,Object> map : resultList) {
            	...
            	xmlBuf.append("<cell><![CDATA[<font color='" + fontColor + "'>" + fail.replace("\"", "\'") + "</font>]]></cell>");
            	xmlBuf.append("</row>");
		...
		try {
			...
			for(int i=0 ; i < item_json.length() ; i++) {
        		...
        		if(seq.length() == 1) {
        			...
        			//System.out.println("chk1 = "+(partNumber+":"+seqStr)+"=>"+ list.contains(partNumber+":"+seqStr));
	...
	public List<Map<String,Object>> partBomListGrid(String oid) throws Exception {
		...
		for(int i=0; i < result.size(); i++) {
			...
			String img = "";
		 	...
		 	String img2 = "";
		 	if(data.level>0){
			    if("join".equals(data.lineImg)){lineStack[data.level]="line";}
			    else lineStack[data.level] = "empty";
		 	...
		 	map.put("productmethod", StringUtil.checkNull(IBAUtil.getAttrValue(data.part, AttributeKey.IBAKey.IBA_PRODUCTMETHOD)));
		 	map.put("model", StringUtil.checkNull(IBAUtil.getAttrValue(data.part, AttributeKey.IBAKey.IBA_MODEL)));
		 	map.put("deptcode", StringUtil.checkNull(IBAUtil.getAttrValue(data.part, AttributeKey.IBAKey.IBA_DEPTCODE)));
		 	map.put("unit", part.getDefaultUnit().toString());
		 	map.put("weight", StringUtil.checkNull(IBAUtil.getAttrfloatValue(data.part, AttributeKey.IBAKey.IBA_WEIGHT)));
		 	map.put("manufacture", StringUtil.checkNull(IBAUtil.getAttrValue(data.part, AttributeKey.IBAKey.IBA_MANUFACTURE)));
		 	...
		 	map.put("specification", StringUtil.checkNull(IBAUtil.getAttrValue(data.part, AttributeKey.IBAKey.IBA_SPECIFICATION)));
	...
	public ResultData updatePackagePartAction(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			for(int i=0; i < modifyChecks.length; i++) {
				...
				//System.out.println("=================================== modifyNumber ===========================   " + modifyNumber);
	...
	public ResultData updateAUIPackagePartAction(Map<String,Object> param) {
		...
		try {
			...
			for(Map<String,Object> itemMap : itemList) {
				...
				//승인된 품목 수정 불가
				/*
				if(partData.isApproved()){
					throw new Exception(partData.number+"는 승인됨 상태라 수정 할수 없습니다.");
				}
				*/
	...
	public Hashtable modify(Map hash, String[] loc, String[] deloc) throws Exception {
		...
		try {
			...
			rtnVal.put("rslt", "F");
			rtnVal.put("msg", e.getLocalizedMessage() == null ? Message.get("수정시 오류가 발생하였습니다.") : e.getLocalizedMessage());
			rtnVal.put("oid", "");
			e.printStackTrace();
		} finally {
			if (trx != null) {
	...
	public Hashtable modify(Map hash, final boolean reviseFlag, String[] loc, String[] deloc) throws Exception {
		...
		try {
			 ...
			 * 
			 * 
			 * String oldNumber = StringUtil.checkNull((String)
			 * hash.get("oldNumber")); //구 번호 String designBy =
			 * StringUtil.checkNull((String) hash.get("designBy")); //설계자 String
			 * drawnDate = StringUtil.checkNull((String) hash.get("drawnDate"));
			 * //설계일자 String surface = StringUtil.checkNull((String)
			 * hash.get("surface")); //표면처리 String heat =
			 * StringUtil.checkNull((String) hash.get("heat")); //열처리 String
			 * processType = StringUtil.checkNull((String)
			 * hash.get("processType")); //가공유형 String maker =
			 * StringUtil.checkNull((String) hash.get("maker")); //제조사 String
			 * comment = StringUtil.checkNull((String) hash.get("comment"));
			 * //설명
			 */
			...
			if (oid.length() > 0) {
				...
				if (!oldPartName.equals(partName)) {
					//E3PSRENameObject.manager.PartReName(part, oldPart.getNumber(), partName, false);
					//E3PSRENameObject.manager.PartReName(part, oldPart.getNumber(), partName, false);
					partReName(part, partName);
					changeNameCheck = true;
				}
			...
			//System.out.println("!!!!!! partMaster.getDefaultUnit().toString() = " + partMaster.getDefaultUnit().toString());
			//System.out.println("!!!!!! Unit									  = " + Unit);
			//if (Unit!=null && !Unit.equals("") && !(partMaster.getDefaultUnit().toString()).equals(Unit)) {
			//엄태식 부장님 설명 듣고 일부러 오류가 발생하도록 재차 수정. 오류 발생 시, ParkCheck = PART 와 SeqCheck = true를 입력 해주면 됨. 관리자 속성 수정 메뉴 사용 (마이그레이션 데이터만 오류가 발생한다 함.)
		...
		if (list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				if ("end".equals(bomType)) {
					...
					//자기자신 제외
					if(part.getNumber().equals(data.number)){
						continue;
					}
					//더미 제외
					if(PartUtil.isChange(data.number)){
						continue;
					}
					
					//마스터 완제품 제외
					if(PartUtil.isProductCheck(data.number) && !PartUtil.completeProductCheck(data.number)){
						continue;
					}
					*/
...
//					map.put("state", data.getLifecycle());
	...
	@Override
	public Map<String, Object> getPartTreeAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
	   ...
	   public  Map<String, Object> getDhtmlXPartData(PartTreeData bomPart, int rowNum) throws Exception{
		   ...
		   Map<String,Object> partData = new HashMap<String, Object>();
			...
			if( (!"ND".equals(dwgNo) || !"AP".equals(dwgNo))&& dwgNo!= null && dwgNo.length() > 0){
				dwgNo = "<a href='#' onclick='javascript:openView(\"" + bomPart.getDwgOid() + "\")'>" + bomPart.getDwgNo() + "</a>";
			}
			...
			String infoIcon = "<img src='/Windchill/netmarkets/images/details.gif'  border=0>^javascript:openView(\"" + oid + "\");^_self";
	...
	public QuerySpec getEPMNumber(String number)throws Exception{
        ...
        try{
        	...
        	if(num != null && num.trim().length() > 0){
            	...
            	query.appendWhere(new SearchCondition(EPMDocument.class, "master>number", SearchCondition.EQUAL , tempNumber), new int[]{idx});
            }
	...
	public String excelBomLoadAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for( int i = 0 ; i<vec.size() ; i++) {
        	...
        	String bom = "";
			if(oid.length()>0){
				bom = "<button type='button' class='btnCustom' onclick=javascript:viewBom('" + oid + "')><span></span>BOM</buttom>";
			}
			...
			for(int i = 0 ;  i < checkOid.length ;  i ++){
				...
				if(productchk != null && StringUtil.checkString(productchk[chkCount])) {
					...
					if(productchk[chkCount].equals(number)) {
						...
						partNumber += seq + etc;
						if(chkCount < (productchk.length-1)) chkCount++;
					}else {
						partNumber = "TEMP";
				...
				String[] partNames = new String[]{partName1, partName2, partName3}; // req.getParameterValues("partName_" + number);
				String partName = "";
				if(partNames != null) {
	...
	@Override
	public Map<String,Object> selectEOPartAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		int page = StringUtil.getIntParameter(request.getParameter("page"), 1);
		int rows = StringUtil.getIntParameter(request.getParameter("rows"), 10);
		int formPage = StringUtil.getIntParameter(request.getParameter("formPage"), 15);
		...
		while(qr.hasMoreElements()){	
			...
			xmlBuf.append("<cell><![CDATA[" + data.createDate.substring(0, 10) + "]]></cell>");
			xmlBuf.append("<cell><![CDATA[" + data.modifyDate.substring(0, 10) + "]]></cell>");
			String bom = "<button type='button' class='btnCustom' onclick=javascript:viewBom('" + data.oid + "')><span></span>BOM</buttom>";
			xmlBuf.append("<cell><![CDATA[" + bom + "]]></cell>");
			xmlBuf.append("</row>");
	...
	@Override
	public Map<String,Object> getBaseLineCompare(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		////System.out.println("getBaselineinforoot2 ="+root2+"," + part2 +"," + bsobj2);
	...
	public  Map<String, Object> getDhtmlXbaselineData(PartTreeData bomPart, PartTreeData bomPart2, int idx, String bgcolor) throws Exception{
	  ...
	  //PJT EDIT 20161129
	   ...
	   // String baseQuantity = "";
	    ...
	    if(bomPart != null){
		    ...
		    if(bomPart.level>0){
	           if("join".equals(bomPart.lineImg)){lineStack[bomPart.level]="line";}
	           else lineStack[bomPart.level] = "empty";
		...
		if(bomPart2 != null){
		    ...
		    if(bomPart2.level>0){
	           if("join".equals(bomPart2.lineImg)){lineStack[bomPart2.level]="line";}
	           else lineStack[bomPart2.level] = "empty";
	 ...
	 * @throws Exception
	 */
	public  ArrayList<Object> getExcelbaselineData(PartTreeData bomPart, PartTreeData bomPart2, int idx, String bgcolor) throws Exception{
	...
	private void createExcelBaseline(ArrayList<Object> rowData,int rowCount,XSSFWorkbook workbook,XSSFCellStyle numberCellStyle,XSSFCellStyle defaultCellStyle){
...
/*	
//	@Override
//	public Map<String,Object> getBaselineinfo(String oid, String oid2, String baseline, String baseline2) throws Exception {
//		
//		Map<String,Object> map = new HashMap<String,Object>();
//		
//		ReferenceFactory rf = new ReferenceFactory();
...
//		
//		if(bsobj2 == null) {
//			title2 = Message.get("최신BOM 전개");
//		}else {
//			title2 = Message.get("Baseline전개")+"-" + bsobj2.getName();
//		}
...
//		    
//		    if(data==null){
//		    	map2.put("isData", isData);
//		    	bgcolor ="gray";
//		    }else {
//		    	if(data2==null){
//		    		map2.put("isData2", isData2);
//		    		bgcolor ="green";
//		    	}else{
//		    		if(!data.compare(data2)){
//		    			bgcolor = "gold";
...
//		    }
//		    
//		    map2.put("bgcolor", bgcolor);
//		    
//		    if(data != null){
//		    	  String icon = CommonUtil.getObjectIconImageTag(data.part);
//				    Map<String,String> data1Map = getBomList(data);
//				    map2.put("icon", icon);
//				    map2.put("data1Map", data1Map);
//		    }
//		  
//		    if(data2 != null){
//		    	 String icon2 = CommonUtil.getObjectIconImageTag(data2.part);
//				    Map<String,String> data2Map = getBomList(data2);
//				    map2.put("icon2", icon2);
//				    map2.put("data2Map", data2Map);
//		    }
//		   
//		    
//		    
...
//	}
//	
//	public Map<String,String> getBomList(PartTreeData data) {
//		String[]  lineStack = new String[50];
//		String img = "";
//		for(int j=1; j< data.level; j++){
//		    
...
//            if("join".equals(data.lineImg)){lineStack[data.level]="line";}
//            else lineStack[data.level] = "empty";
//            img += "<img src='/Windchill/jsp/part/images/tree/" + data.lineImg + ".gif' border=0></img>";
//	    }
//	    
//	    Map<String,String> map = new HashMap<String,String>();
//	    map.put("img", img);
...
//	    map.put("itemSeq", data.itemSeq);
//	    map.put("baseQuantity", String.valueOf(data.baseQuantity));
//	    return map;
//	    
//	}
*/	
	...
	public List<Map<String,Object>> viewPartBomAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		/*
		if(isTop){
			partTrees = BomSearchHelper.service.getNextBOM(part, boolDesc, bsobj, false, partTrees);
		}else{
		...
		*/
		...
		partTrees = BomSearchHelper.service.getBOM(part, boolDesc, bsobj, isTop, parentId);//(part, boolDesc, bsobj,parentID);
		...
		for(PartTreeData data : partTrees) {
			...
			if( (!"ND".equals(dwgNo) || !"AP".equals(dwgNo))&& dwgNo!= null && dwgNo.length() > 0){
				dwgNo = "<a href='#' onclick='javascript:openView(\"" + data.getDwgOid() + "\")'>" + data.getDwgNo() + "</a>";
			}
			...
			map.put("deptcode", NumberCodeHelper.service.getValue(AttributeKey.IBAKey.IBA_DEPTCODE, data.getDeptcode()));
			
			boolean isChildren = BomSearchHelper.service.isChildren(data.part, boolDesc, bsobj);//isChildren(data.part, desc, baseline);
	...
	public List<Map<String,Object>> listAUIPartAction(HttpServletRequest request, HttpServletResponse response)
		...
		while (qr.hasMoreElements()) {
			...
			o = (Object[]) qr.nextElement();
			part = (WTPart) o[0];
			System.out.println("part number : " + part.getNumber() + " / " + part.getPersistInfo().getObjectIdentifier().getStringValue());
			data = new VersionData((RevisionControlled) part);
			String remarks =  StringUtil.checkNull(IBAUtil.getAttrValue((IBAHolder) part, AttributeKey.IBAKey.IBA_REMARKS));
...
//			}
	 ...
	 */
	@Override
	public void partTreeSelectAttachDown(HttpServletRequest request,HttpServletResponse response,Map<String,Object> param) throws Exception{
		...
		for(Map<String,Object> rowMaps : rowItems) {
			...
			listAppData = CommonContentHelper.service.getAttachFileList((ContentHolder)part, ContentRoleType.SECONDARY.toString(), listAppData);
	...
	public ResultData batchBomDrawingDownAction(String oid, String describe,String ecoOid) {
		...
		try{
			...
			//}
			//System.out.println("batchBomDrawingDownAction baseline Name="+baseline.getName());
			...
			//HashMap<ApplicationData, String> mapTemp = new HashMap<ApplicationData, String>();
	 ...
	 */
	@Override
	public List<BatchDownData> getEPMBatchDownList(List<BatchDownData> list,List<WTPart> partList,String describe) throws Exception{
		
		for(WTPart part : partList){
			
			EPMDocument epm = DrawingHelper.service.getEPMDocument(part);
			//System.out.println("part number = "+ part.getNumber()+"\t Start");
			//System.out.println("epm number = "+ (null!=epm?epm.getNumber():"Null")+"\t Start");
			if(epm != null){
					...
					if(vec2D.size() ==0){
						...
						for(int h = 0 ; h < vec2D.size() ; h++){
							...
							if(epm2d.getDocType().toString().equals("CADDRAWING")){
								list = CommonContentHelper.service.getAttachFileList((ContentHolder)epm2d, list, describe);
							}
	 ...
	 */
	@Override
	public ResultData batchBomSelectDownAction(String oid, List<Map<String,Object>> itemList ,String describe,String downType,String describeType) {
		ResultData returnData = new ResultData();
		try{
			WTPart part = (WTPart)CommonUtil.getObject(oid);
			List<BatchDownData> list = new ArrayList<BatchDownData>();
	...
	private ArrayList descentLastPart(WTPart part, Baseline baseline, State state)
			throws WTException {
		...
		try {
			...
			while (re.hasMoreElements()) {
				...
				////System.out.println("baseline1111 : "+subPart.getNumber() +":"+link.getQuantity().getAmount());
				v.add(oo);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private ArrayList descentLastPart(WTPart part, View view, State state)
			throws WTException {
		ArrayList v = new ArrayList();
		if (!PersistenceHelper.isPersistent(part))
			return v;
	...
	public ResultData updateAUIPartChangeAction(Map<String, Object> param) {
		...
		try {
			...
			//System.out.println("Before Oid = "+CommonUtil.getOIDString(pPart));
			 ...
			 if(paramListdata.size()>0){
				 for (int i = 0; i < paramListdata.size(); i++) {
					 ...
					 int idx = 0;
					  storeNumberANameValue(cOid, realPartNumber, realPName,realPName1,realPName2,realPName3,realPName4,isPartNameEdit);
				}
			}else{
			...
			if(numberFormCheck(msg)) msg = "알수 없는 오류가 발생하였습니다. 관라자에게 문의 하세요.";
			data.setMessage(msg);
			e.printStackTrace();
		}finally {
			if(trx != null) {
		...
		for (int i = 0; i < oidList.size(); i++) {
    		...
    		if(null!=realPNumber && realPNumber.length()==10 && isNumberFormCheck){
    			...
    			if(seqStr.equals("000")){
    				...
    				/* if(null!=partNumberSeqs &&partNumberSeqs.length()>0 ){
    					 //seq= String.valueOf((Integer.parseInt(seq)+partNumberSeqsint));
    					 //System.out.println((gubun+main+middle)+"\tpartNumberSeqs="+partNumberSeqs);
    					 seq= String.valueOf((Integer.parseInt(seq)+Integer.parseInt(partNumberSeqs)));
    				 }else{*/
    					 int gubunIDx = 0;
    					 //System.out.println("fiveNumberCheckList Size = "+ fiveNumberCheckList.size());
	...
	private void storeNumberANameValue(String changeOid, String realPNumber, String partName,String partName1,String partName2,String partName3,String partName4,boolean isPartNameEdit) throws Exception {
	...
	private String checkExtention(String extention, String docTypeEng) {
		if(docTypeEng.equals("CAD PART") && !extention.toUpperCase().equals("PRT")) extention = "PRT";
		if(docTypeEng.equals("ASSEMBLY") && !extention.toUpperCase().equals("ASM")) extention = "ASM";
		if(docTypeEng.equals("DRAWING") && !extention.toUpperCase().equals("DRW")) extention = "DRW";
		return extention;
	}
