...
public class StandardPartQueryservice extends StandardManager implements PartQueryservice {
	...
	public QuerySpec listPartSearchQuery(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String productmethod = StringUtil.checkNull(request.getParameter("productmethod")); // 제작방법 (NumberCode,
																								// IBA)
			...
			String manufacture = StringUtil.checkNull(request.getParameter("manufacture")); // MANUTACTURE (NumberCode,
																							// IBA)
			...
			// 배포 관련 추가
			boolean isProduction = StringUtil.checkNull(request.getParameter("production")).equals("true") ? true
					: false;
			boolean ischeckDummy = StringUtil.checkNull(request.getParameter("checkDummy")).equals("true") ? true
					: false;
			...
			if (ecoPostdate.length() > 0 || ecoPredate.length() > 0) {
				...
				/*
				 * if(ecoPredate.length() > 0 ){ query.appendAnd(); query.appendWhere(new
				 * SearchCondition(StringValue.class, "value",
				 * SearchCondition.GREATER_THAN_OR_EQUAL, ecoPredate), new int[] { _idx }); }
				 * 
				 * if(ecoPostdate.length() > 0 ){ query.appendAnd(); query.appendWhere(new
				 * SearchCondition(StringValue.class, "value",
				 * SearchCondition.LESS_THAN_OR_EQUAL, ecoPostdate), new int[] { _idx }); }
				 * SELECT A0.ida2a2,A1.value , SUBSTR(A1.value,INSTR(A1.value,',',-1)+1),
				 * INSTR(A1.value,',',-1) FROM WTPart A0,StringValue A1,WTPARTMASTER A2 WHERE
				 * (A0.latestiterationInfo = 1) AND (UPPER(A0.statecheckoutInfo) <> 'WRK') AND
				 * (A0.statestate = 'APPROVED') AND (A0.IDA3MASTERREFERENCE=A2.IDA2A2) AND
				 * (A1.idA3A4 = A0.idA2A2) AND (A1.hierarchyIDA6 = '3899120390680943734') AND
				 * (SUBSTR(A1.value,INSTR(A1.value,',',-1)+1) >= '2017-07-26') AND
				 * (A2.WTPartNumber NOT LIKE '%DEL%') ORDER BY A0.modifyStampA2 DESC;
				 *
				 *
				 *
				 */
			...
			if (sortValue.length() > 0) {
				...
				/*
				 * if ("true".equals(sortCheck)) { if (!"creator".equals(sortValue)) {
				 * query.appendOrderBy(new OrderBy(new ClassAttribute(WTPart.class, sortValue),
				 * true), new int[] { idx }); } else { if (query.getConditionCount() > 0)
				 * query.appendAnd(); int idx_user = query.appendClassList(WTUser.class, false);
				 * int idx_people = query.appendClassList(People.class, false);
				 * 
				 * ClassAttribute ca = null; ClassAttribute ca2 = null;
				 * 
				 * ca = new ClassAttribute(WTPart.class, "iterationInfo.creator.key.id"); ca2 =
				 * new ClassAttribute(WTUser.class, "thePersistInfo.theObjectIdentifier.id");
				 * 
				 * SearchCondition sc2 = new SearchCondition(ca, "=", ca2);
				 * 
				 * query.appendWhere(sc2, new int[] { idx, idx_user });
				 * 
				 * ClassAttribute ca3 = new ClassAttribute(People.class,
				 * "userReference.key.id");
				 * 
				 * query.appendAnd(); query.appendWhere(new SearchCondition(ca2, "=", ca3), new
				 * int[] { idx_user, idx_people }); SearchUtil.setOrderBy(query, People.class,
				 * idx_people, People.NAME, "sort", true); }
				 * 
				 * } else { if (!"creator".equals(sortValue)) { query.appendOrderBy(new
				 * OrderBy(new ClassAttribute(WTPart.class, sortValue), false), new int[] { idx
				 * }); } else { if (query.getConditionCount() > 0) query.appendAnd(); int
				 * idx_user = query.appendClassList(WTUser.class, false); int idx_people =
				 * query.appendClassList(People.class, false);
				 * 
				 * ClassAttribute ca = null; ClassAttribute ca2 = null;
				 * 
				 * ca = new ClassAttribute(WTPart.class, "iterationInfo.creator.key.id"); ca2 =
				 ...
				 * query.appendAnd(); query.appendWhere(new SearchCondition(ca2, "=", ca3), new
				 * int[] { idx_user, idx_people }); SearchUtil.setOrderBy(query, People.class,
				 * idx_people, People.NAME, "sort", false); } }
				 */
			} else {
	...
	public QuerySpec listPartApprovedSearchQuery(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		QuerySpec query = new QuerySpec();
		int idx = query.addClassList(WTPart.class, true);
		ReferenceFactory rf = new ReferenceFactory();
