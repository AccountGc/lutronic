...
//            CommonUtil.setOrderBy(spec, mainClass, mainClassPos, Department.CODE, "code", false);
    ...
    public QuerySpec getChildQuerySpec(Department tree) throws WTPropertyVetoException, QueryException,
            WTIntrospectionException
    {
        QuerySpec spec = new QuerySpec();
        Class mainClass = Department.class;
        int mainClassPos = spec.addClassList(mainClass, true);
        spec.appendWhere(new SearchCondition(mainClass, "parentReference.key.id", SearchCondition.EQUAL, CommonUtil
                .getOIDLongValue(tree)), mainClassPos);
//        CommonUtil.setOrderBy(spec, mainClass, mainClassPos, Department.CODE, "code", false);
        SearchUtil.setOrderBy(spec, mainClass, mainClassPos, "thePersistInfo.createStamp", "createStamp", false);
        return spec;
    }
    ...
    public Department getDepartment(String code)
    {
        try
        {
            ...
            int mainClassPos = spec.addClassList(mainClass, true);
            spec
                    .appendWhere(new SearchCondition(mainClass, Department.CODE, SearchCondition.EQUAL, code),
                                 mainClassPos);
            QueryResult qr = PersistenceHelper.manager.find(spec);
            if (qr.hasMoreElements())
            {
