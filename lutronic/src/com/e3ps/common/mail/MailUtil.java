...
//			System.out.println("@@ call sendMailFtp()  title = " + subject);
			...
			for(int i=0; i< toId.length; i++){
				...
				if(toUser.getEMail()==null)continue;
...
/*	public static void mailObjMailSendSetting(ApprovalLine nextLine, Object object, HashMap toHash, String title, String msg, String mailType) throws Exception{
	...
	public static void mailObjMailSendSetting(ApprovalLine nextLine, Object object, HashMap toHash, String title, String msg, String mailType,boolean isWork) throws Exception{
		...
		String link = host;
		if( nextLine != null ) { 
			link += WebUtil.getHost()+"jsp/workspace/approval/ViewListWork.jsp?view=toApprove&module=workspace&oid=" + nextLine.getPersistInfo().getObjectIdentifier().toString();
		} else {
			link += WebUtil.getHost()+"jsp/workspace/approval/ViewListWork.jsp?view=toApprove&module=workspace&oid=" + ((Persistable)object).getPersistInfo().getObjectIdentifier().toString();
		}
...
//		System.out.println("Mail 발송 : " + result);
	...
	public boolean taskNoticeMail(WorkItem item) {
		...
		try {
			...
			if (toEmail == null || toEmail.length() == 0) {
				...
				if ("auto".equals(company))
					toEmail = wp.getName() + "@" + "i-jy.com";
				else
					toEmail = wp.getName() + "@" + "i-jy.com";*/
			}
			...
			//System.out.println("admin-->> " + adminUser.getFullName() + " : " + adminUser.getEMail());
	...
	public void sendeOutSideMail(WTObject obj,HashMap mailUserMap) {
		...
		//System.out.println("[MailUtil.manager.sendeOutSideMail : mailUserMap =" +mailUserMap.size());
		//System.out.println("[MailUtil.manager.sendeOutSideMail : enableMail =" +enableMail);
		wt.org.WTUser adminUser = null;
		String mcontent = "";
		String subject = "";
		String workName ="";
