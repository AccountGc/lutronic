...
public class StandardCodeService extends StandardManager implements CodeService {
	...
	@Override
	public List<NumberCodeData> numberCodeList(String codeType, String parentOid, boolean search) {
	...
	@Override
	public List<NumberCodeData> numberParentCodeList(String codeType, String pCode, boolean search) {
		List<NumberCodeData> list = new ArrayList<NumberCodeData>();
	...
	private QuerySpec numberCodeListQuery(String codeType, String parentOid, boolean search) {
		...
		try {
			...
			if(!search) {
				query.appendAnd();
				query.appendWhere(new SearchCondition(NumberCode.class, "disabled", SearchCondition.IS_FALSE), new int[] { idx });
			}
			query.appendAnd();
			...
			query.appendOrderBy(new OrderBy(new ClassAttribute(NumberCode.class, NumberCode.SORT), false), new int[] { idx });
			query.appendOrderBy(new OrderBy(new ClassAttribute(NumberCode.class, NumberCode.NAME), false), new int[] { idx });
	...
	private QuerySpec numberParentCodeListQuery(String codeType, String pCode, boolean search) {
		...
		try {
			...
			if(pCode.length()>0){
				query.appendAnd();
				query.appendWhere(new SearchCondition(NumberCode.class, NumberCode.CODE, SearchCondition.EQUAL, pCode), new int[] { idx });
			}
	...
	@Override
	public List<NumberCodeData> autoSearchName(String codeType, String name) {
		List<NumberCodeData> list = new ArrayList<NumberCodeData>();
		...
		while(qr.hasMoreElements()) {
			...
			NumberCodeData data = new NumberCodeData(numbercode);
			list.add(data);
		}
	 ...
	 */
	@Override
	public Map<String,Object> popup_numberCodeAction(HttpServletRequest request, HttpServletResponse response,boolean disabled) throws Exception {
		...
		if(StringUtil.checkString(sessionId)) {
		    ...
		    if(code.length()>0){
		    	query.appendAnd();
		    	query.appendWhere(new SearchCondition(NumberCode.class,NumberCode.CODE,SearchCondition.LIKE,"%"+code+"%",false),new int[] {0});
		    }
	...
	public QueryResult getCode(String key){
		try{
			...
			if(key.equals("CADCREATOR")){
				query.appendOrderBy(new OrderBy(new ClassAttribute(NumberCode.class,NumberCode.ENG_NAME),false),new int[]{0});
			}else{
				query.appendOrderBy(new OrderBy(new ClassAttribute(NumberCode.class,NumberCode.CODE),false),new int[]{0});
			}
	...
	@Override
	public List<NumberCodeData> topCodeToList(String key) {
		List<NumberCodeData> list = new ArrayList<NumberCodeData>();
	...
	@Override
	public List<NumberCodeData> childNumberCodeList(String type, String parentCode) {
		List<NumberCodeData> list = new ArrayList<NumberCodeData>();
	...
	public boolean isUseCheck(NumberCode code){
		...
		try{
			if(code.getCodeType().equals("EOTYPE")){					//(ECO)
				...
				QuerySpec qs =  new QuerySpec(EChangeOrder.class);
				qs.appendWhere(new SearchCondition(EChangeOrder.class,"ecoType",SearchCondition.EQUAL,code.getCodeType().toString()));
				QueryResult rt = PersistenceHelper.manager.find(qs);
				if(rt.size() >0 ); isUse =true;
			
			}else if(code.getCodeType().equals("CHANGEPURPOSE")){		//(ECO,ECR)
			
				/*ECO*/
				QuerySpec qs =  new QuerySpec(EChangeOrder.class);
				qs.appendWhere(new SearchCondition(EChangeOrder.class,"purpose",SearchCondition.LIKE,"%"+code.getCodeType().toString()+"%"));
				QueryResult rt = PersistenceHelper.manager.find(qs);
				if(rt.size() >0 ); isUse =true;
				
				/*ECR*/
				qs =  new QuerySpec(EChangeRequest.class);
				qs.appendWhere(new SearchCondition(EChangeRequest.class,"purpose",SearchCondition.LIKE,"%"+code.getCodeType().toString()+"%"));
				...
				/*ECO*/
				QuerySpec qs =  new QuerySpec(EChangeOrder.class);
				qs.appendWhere(new SearchCondition(EChangeOrder.class,"stockPart",SearchCondition.LIKE,"%"+code.getCodeType().toString()+"%"));
				QueryResult rt = PersistenceHelper.manager.find(qs);
				if(rt.size() >0 ); isUse =true;
			}
		}catch(Exception e){;
