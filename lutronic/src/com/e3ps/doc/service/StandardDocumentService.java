...
public class StandardDocumentService extends StandardManager implements DocumentService {
	...
	@Override
	public Map<String,Object> listDocumentAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		while(qr.hasMoreElements()){	
			...
			if(data.getDocumentType().equals("금형문서")) {
				...
				xmlBuf.append("<cell><![CDATA[<a href=javascript:openView('" + data.oid + "')>" + data.name + "</a>]]></cell>" );
				xmlBuf.append("<cell><![CDATA[" + data.location.substring(("/Default/Document").length()) + "]]></cell>" );
				xmlBuf.append("<cell><![CDATA[" + data.version + "." + data.iteration + "]]></cell>" );
				xmlBuf.append("<cell><![CDATA[" + data.getLifecycle() + "]]></cell>" );
	 ...
	 */
	@Override
	public List<Map<String,Object>> listAUIDocumentAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	@Override
	public Map<String, Object> listPagingAUIDocumentAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
	        ...
	        if(oid != null){
	            ...
	            if (!doc.getName().equals(documentName)) {
	                ...
	                if("$$MMDocument".equals(doc.getDocType().toString())) {
	                	identity.setName(docName);
	                }else {
	                	if(StringUtil.checkString(docName)) {
	                		identity.setName(documentName + "-" + docName);
	                	}else {
			...
			for(int i=0 ; i < item_json.length() ; i++) {
				...
				if(partNumber.length() > 0) {
					...
					for(int j=0; j<partNumbers.length; j++) {
						...
						if(part == null) {
							checkPart = false;
							//System.out.println("Part is Null partNumbers+["+j+"]" + partNumbers[j] +","+part);
				...
				try{
					...
					e.printStackTrace();
					//System.out.println("ERRROR : " + e.getMessage() +"::::"+ e.getLocalizedMessage());
					totalValidation = false;
					e.printStackTrace();
	...
	public synchronized String delete(String oid) throws Exception {
        ...
        try{
            ...
            if(oid != null){
                ...
                int index0 = spec.addClassList(WTPartDescribeLink.class, true);
                int index1 = spec.addClassList(WTDocument.class, false);

                ClassAttribute ca0 = new ClassAttribute(WTPartDescribeLink.class, "roleBObjectRef.key.id");
                ClassAttribute ca1 = new ClassAttribute(WTDocument.class, "thePersistInfo.theObjectIdentifier.id");
                SearchCondition sc = new SearchCondition(ca0, "=", ca1);
                spec.appendWhere(sc, new int[]{index0, index1});
                spec.appendAnd();
                spec.appendWhere(new SearchCondition(WTDocument.class, "master>number", "=", wtdoc.getNumber()), new int[]{index1});
                ...
                */
                ...
                QueryResult results = PersistenceHelper.manager.navigate(wtdoc, "describes", WTPartDescribeLink.class, false);
                if(results.size() > 0) {
                	throw new Exception(Message.get("관련 품목") + Message.get("이(가) 있을 때 삭제할 수 없습니다."));
                }
                
                List<DocumentToDocumentLink> used = DocumentQueryHelper.service.getDocumentToDocumentLinks(wtdoc, "used");
                List<DocumentToDocumentLink> useBy = DocumentQueryHelper.service.getDocumentToDocumentLinks(wtdoc, "useBy");
    ...
    public WTDocument getLastDocument(String number) throws Exception{
    	...
    	try{
	    	...
	    	query.appendAnd();
	    	query.appendWhere(new SearchCondition(WTDocument.class, "master>number",SearchCondition.EQUAL,number,false),new int[]{idx});
    ...
    @Override
    public Map<String,Object> createDocumentLinkAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(StringUtil.checkString(sessionId)) {
			...
			//QuerySpec query = DocumentQueryHelper.service.listCreateDocumentLinkAction(request, response);
			QuerySpec query = DocumentQueryHelper.service.getListQuery(request, response);
			qr = PageQueryBroker.openPagingSession((page - 1) * rows, rows, query, true);
		}
		...
		while(qr.hasMoreElements()){	
			...
			//xmlBuf.append("<cell><![CDATA[<a href=" + data.pUrl + ">" + data.icon + "]]></cell>" );
			xmlBuf.append("</row>" );
    ...
    public Vector<WTPartDescribeLink> getWTPartDescribeLink(RevisionControlled rc,boolean isLast){
    	...
    	try{
    		...
    		//Join
    		qs.appendWhere(new SearchCondition(WTPartDescribeLink.class,"roleBObjectRef.key.id", WTDocument.class,"thePersistInfo.theObjectIdentifier.id"),new int[]{idxA,idxB});
    		
    		qs.appendAnd();
    		qs.appendWhere(new SearchCondition(WTPartDescribeLink.class,"roleAObjectRef.key.id", WTPart.class,"thePersistInfo.theObjectIdentifier.id"),new int[]{idxA,idxC});
    		
    		qs.appendAnd();
    		qs.appendWhere(new SearchCondition(WTPartDescribeLink.class,columnName, SearchCondition.EQUAL,logOid),new int[]{idxA});
    ...
    public ResultData deleteDocumentLinkAction(HttpServletRequest request, HttpServletResponse response) {
    	...
    	try {
    		...
    		/*
    		if("active".equals(type)) {
        		QuerySpec spec = DocumentQueryHelper.service.devActiveLinkDocument(parentOid, docOid);
    ...
    public void deleteDocumentToPartLink(WTDocument document) throws Exception {
    	QueryResult results = PersistenceHelper.manager.navigate(document, "describes", WTPartDescribeLink.class, false);
     ...
     * @throws Exception
     */
    public void updateDocumentToDevelopmentLink(WTDocument document, String[] activeOids, boolean isDelete) throws Exception {
    	if(isDelete) {
