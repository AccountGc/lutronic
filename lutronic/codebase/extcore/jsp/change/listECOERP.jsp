...
$(function(){
	...
	*                      ERP 전송 현황 체크
	...
	$("#ERPCheckBtn").click(function (){
		...
		var form = $("form[name=ecoERPForm]").serialize();
...
function  lfn_Search(){
	...
	var form = $("form[name=ecoERPForm]").serialize();
	var url	= getURLString("erp", "listECOERPAction", "do");
	$.ajax({
		type:"POST",
		url: url,
		data:form,
...
function lfn_DhtmlxGridInit() {
	var COLNAMEARR = {
			...
			,col03 : '${f:getMessage("변경번호")}'			//sort  aennr  col03
			,col04 : '${f:getMessage("변경내역(제목)")}'	//sort  aetxt  col04
			,col05 : '${f:getMessage("효력시작일")}'
			,col06 : '${f:getMessage("변경사유")}'
			,col07 : '${f:getMessage("EO/ECO 구분")}'
			,col08 : '${f:getMessage("인터페이스상태")}'   //sort  zifsta  col28
			,col09 : '${f:getMessage("인터페이스결과")}'
			,col10 : '${f:getMessage("전송일")}'		   //sort  createstamp  col10
			,col11 : 'ERP ${f:getMessage("적용 상태")}'
			,col12 : 'ERP ${f:getMessage("적용 결과")} '
	}
	...
	documentListGrid.attachEvent("onHeaderClick",function(rowId,cellIndex){
		...
		}else if(rowId == 2 ){		  //변경번호
			sortValue = "aennr";
			header = COLNAMEARR.col03;
		}else if(rowId == 3 ) {
			sortValue = "aetxt";		//변경내역
		...
		if(sortValue != "") {
			...
			documentListGrid.setColLabel(910, COLNAMEARR.col11);
...
<body>
<form name="ecoERPForm" id="ecoERPForm" method=post style="padding:0px;margin:0px">
				...
				<table cellspacing="0" cellpadding="0" border=0 width=100% height=29 class="Subinfo_img_bg">
					<tr>
						...
						<td>
							...
							&nbsp; ERP ${f:getMessage('전송')}&nbsp;${f:getMessage('현황')} > EO&ECO ${f:getMessage('전송')}&nbsp;${f:getMessage('현황')}
						</td>
					</tr>
				</table>
			</td>
		...
		<td valign="top" style="padding:0px 0px 0px 0px">
			...
			<col width='10%'><col width='40%'><col width='10%'><col width='40%'>
				<tr>
					...
					<td class="tdblueM">${f:getMessage("변경번호")}&nbsp;${f:getMessage("번호")}</td>
					<td class="tdwhiteL">
						<input name="aennr" id="aennr" class="txt_field" size="30" style="width:90%" value=""/>
					</td>
				</tr>
				<tr>
					<td class="tdblueM">${f:getMessage("변경내역(제목)")}</td>
					<td class="tdwhiteL" colspan="3">
						<input name="aetxt" id="aetxt" class="txt_field" size="30" style="width:90%" value=""/>
					</td>
