					...
					<tbody id="ecaTableBody">
						<c:choose>
							<c:when test="${fn:length(list) != 0 }">
								<c:forEach items="${list}" var="ecrrData"  varStatus="status">
									<tr>
										<td class="tdwhiteM">
											<c:out value="${ecrrData.number }" />
										</td>
										<td class="tdwhiteM">
											<c:choose>
												<c:when test="${distribute eq 'true' }">
														<c:choose>
															<c:when test="${ecrrData.isApproved()}">
																<a href="javascript:openDistributeView('<c:out value="${ecrrData.oid }" />')">
																	<c:out value="${ecrrData.name }" />
																</a>
															</c:when>
															<c:otherwise>
																<c:out value="${ecrrData.name }" />
												...
												<c:otherwise>
													<a href="javascript:openView('<c:out value="${ecrrData.oid }" />')">
														<c:out value="${ecrrData.name }" />
													</a>
												</c:otherwise>
											</c:choose>	
										...
										<td class="tdwhiteM">
											<c:out value="${ecrrData.lifecycle }" />
										</td>
										<td class="tdwhiteM">
											<c:out value="${ecrrData.creator }" />
										</td>
										<td class="tdwhiteM">
											<c:out value="${ecrrData.approveDate }" />
										</td>
									</tr>
								</c:forEach>
							</c:when>
