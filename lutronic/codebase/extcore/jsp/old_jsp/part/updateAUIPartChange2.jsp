...
$(document).ready(function() {
	...
	AUIGrid.bind(myBOMGridID, "cellEditBegin", function( event ) {
		console.log("You clicked cell : " + event.dataField+"\t"+event.rowIndex);
		if(event.dataField == "partNameSeq" || event.dataField == "etc") {
			if(event.item.isDisablePartType && event.item.isCheckPartType!="가능" && (null==event.item.realPartNumber)) {
				console.log(event.dataField+" false 반환. 기본 행위인 편집 불가");
		...
		if(event.dataField == "partName1" || event.dataField == "partName2" ||
			...
			console.log("2 State="+event.item.partState);
	...
	AUIGrid.bind(myBOMGridID, ["cellEditEnd","cellClick"], function( event ) {
       if(event.type == "cellClick"){ 
    	   console.log("You clicked cell : " + event.value+"\trowIndex="+event.rowIndex);
       }else if(event.type == "cellEditEnd"){ 
    	   console.log("You cellEditEnd : " + event.dataField);
    	   if(event.dataField=="isCheckPartType"){
    		   //console.log("You cellEditEnd isCheckPartType : " + event.value);
    		   checkboxChangeHandler(event);
...
$(function() {
	...
	$("#setNumber").click(function() {
		...
		$("input[type=hidden]").each(function() {
			if($.trim(this.name) != "") {
				console.log("param => "+this.name +"="+this.value);
			...
			$.ajax({
				...
				success:function(data){
					if(data.result) {
						...
						location.href = getURLString("part", "updateAUIPartChange", "do") + "?partOid=" + data.oid;
					}else {
...
function apply(dataMap){
	...
	console.log("change  ========== "+gubun+","+gubunOid+","+main+","+mainOid+","+middle);
	...
	for(var i = 0; i < checkedItems.length; i++){
		...
		if(isCheckPartType=="가능") {
			...
			console.log("update  ==========rowId = "+rowId+","+changegubun+","+changemain+","+changemiddle+","+changeseqs);
...
function validationCheck() {
	...
	if(checkedItems.length == 0){
		...
		for(var i = 0; i < checkedItems.length; i++){
			...
			if(isCheckPartType=="가능"){
				console.log("1 step : "+(gubun == ""));
				if(gubun == ""){
					alert("["+rowId+"]"+number+"의  ${f:getMessage('품목구분')}${f:getMessage('을(를) 선택하세요.')}");
					return true;
				}
				console.log("2 step : "+(main == ""));
				if(main == ""){
					alert("["+rowId+"]"+number+"의    ${f:getMessage('대분류')}${f:getMessage('을(를) 선택하세요.')}");
					return true;
				}
				console.log("3 step : "+(middle == ""));
				if(middle == ""){
					alert("["+rowId+"]"+number+"의   ${f:getMessage('중분류')}${f:getMessage('을(를) 선택하세요.')}");
					return true;
				}
				console.log("4 step : "+(typeof(realPartNumber)!="undefined" && null!=realPartNumber && realPartNumber.length>=5));
				console.log("5 step : "+(typeof(realPartNumber)!="undefined" && null!=realPartNumber && realPartNumber.length<5));
				if(typeof(realPartNumber)!="undefined" && null!=realPartNumber && realPartNumber.length>=5){
					...
					console.log("5-1 step : partNumberSeqs : "+partNumberSeqs);
					...
					console.log("ADD INPUT HIDDEN "+i);
			...
			//console.log("realPartName="+realPartName);
			console.log("6 step : "+(typeof(realPartName)!="undefined" && null!=realPartName));
			console.log("7 step : "+(typeof(realPartNumber)=="undefined"));
...
function check(tmp){
	...
	console.log("change data1 = "+data);
	if(data.lastIndexOf("_")==(data.length-1)){
		data = data.substring(0,data.length-1);
	}
	console.log("change data2 = "+data);
	if(data.indexOf("_")==(0)){
...
function getParentV(objList,selectIdx){
	var pOIDreturn = "";
	console.log("objList Start");
	console.log(objList);
	console.log("objList End");
	 for(var i=0,len=objList.length; i<len; i++) {
		 console.log("objList["+i+"].value="+objList[i].value)
 		if(selectIdx == objList[i].code){
...
function selectboxChangeHandler(event,field) {
	if(event.type == "cellEditEnd") {
		...
		if((event.dataField=="gubun" || event.dataField=="main" || event.dataField=="middle" )&& event.value.length>0 ){
			console.log(111232323231111111111111111);
			var parentOid = "";
			var parentValue ="";
			var objList;
			var selectIdx;
			...
			//console.log(typeof(rowItem.partNumberSeqs)!="undefined");
			//console.log(rowItem.partNumberSeqs.length>0);
			//console.log(rowItem.partNumberSeqs);
			//console.log(rowItem.partNumberSeqs.length);
    		if(typeof(rowItem.partNumberSeqs)!="undefined" && rowItem.partNumberSeqs.length>0){
    			console.log("rowItem.partNumberSeqs="+rowItem.partNumberSeqs);
    			rowItem.partNumberSeqs = "";
    			console.log("c rowItem.partNumberSeqs="+rowItem.partNumberSeqs);
    		}
			...
			console.log(partNumber4);
			console.log(partNumber5);
			...
			var partNumber4 = changeString(rowItem.partNameSeq,3);
			var partNumber5 = changeString(rowItem.etc,2);
			console.log(event.dataField+"\tpartNumber4.length="+partNumber4.length);
			console.log("partNumber5.length="+partNumber5.length);
...
function viewAUIPartBomAction(){
	...
	var url	= getURLString("part", "updateAUIPartChangeSearchAction", "do");
	$.ajax({
		type:"POST",
		url: url,
		data : form,
...
<input type="hidden" name="oid" id="oid" value="<c:out value='${partOid }'/>">
