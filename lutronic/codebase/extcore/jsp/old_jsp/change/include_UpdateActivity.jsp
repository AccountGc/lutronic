...
$(document).ready(function() {
	...
	var eoOid = "<c:out value='${eoOid}'/>";
	EOActivityLoad(eoType, eoOid);
	...
	*                      각 활동에 대한 체크박스 동작
	...
	$("input[name=activity]").click(function() {
		...
		if("true" == $("#"+id).attr("isNeed")) {
			alert($("#"+id).attr("activeName")+" ${f:getMessage('변경은 필수 사항입니다.')}");
			this.checked = true;
		}
	})
	<%----------------------------------------------------------
...
----------------------------------------------------------%>
function EOActivityLoad(eoType, eoOid) {
	var url	= getURLString("change", "getEOActivityUpdateList", "do");
	$.ajax({
		type:"POST",
		url: url,
		data:{eoType:eoType, eoOid:eoOid},
		dataType:"json",
		async: false,
		cache: false,
	...
	<tr>
		<td>
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<td width="*">
						<img src="/Windchill/jsp/portal/img/bt_01.gif" > <b>${f:getMessage('활동 설정')} </b>
					</td>
				</tr>
			</table>
                 	...
                 	<td class="tdwhiteL" colspan=3>
                 		<table width="100%" border="0" cellpadding="0" cellspacing="0" align=center  !style="tdisplay:none;able-layout:fixed">
							<tr>
								...
								<td  class="tdblueM"  width=20%>${f:getMessage('담당 부서')}</td>
