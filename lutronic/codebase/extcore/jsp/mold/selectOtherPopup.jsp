...
$(document).ready(function(){
	...
	documentType('GENERAL', $('#searchType').val());
	numberCodeList('manufacture', '');
...
window.documentType = function(documentType, searchType) {
	...
	var data = common_documentType('GENERAL', searchType);
...
function lfn_DhtmlxGridInit() {
	...
	if(type =="ROHS"){
		COLNAMEARR = {
				...
				,col03 : '${f:getMessage('물질명')}${f:getMessage('번호')}'
		...
		var COLNAMEARR = {
			 	col01 : '<input type=checkbox name=allCheck id=allCheck>'
		 		,col02 : '${f:getMessage('번호')}'
				,col03 : '${f:getMessage('문서')}${f:getMessage('번호')}'
				,col04 : '${f:getMessage('문서명')}'
		...
		if(type == "select") {
			...
			for(var i =0; i < array.length; i++) {
				...
				/*
	  			...
	  			*/
	  			var number;var manufacture ;var name;var version;var state;var creator;var createDate;var updateDate
	  			if(searchType == "ROHS"){
	  				number = documentListGrid.cells(array[i], 2).getValue();				     // 물질번호
	  				manufacture = documentListGrid.cells(array[i], 2).getValue();				 // 협력 업체
	  				name = documentListGrid.cells(array[i], 4).getValue();				         // 물질명
		   			version = documentListGrid.cells(array[i], 5).getValue();				     // 버전
		   			state = documentListGrid.cells(array[i], 6).getValue();	 		 	         // 상태
		   			creator = documentListGrid.cells(array[i], 7).getValue();     			     // 등록자
		 			createDate = documentListGrid.cells(array[i], 8).getValue();			 	 // 등록일
		  			updateDate = documentListGrid.cells(array[i], 9).getValue();			 	 // 수정일
	  			}else{
	  				number = documentListGrid.cells(array[i], 2).getValue();				     // 문서번호
		   			name = documentListGrid.cells(array[i], 3).getValue();				         // 문서명
		   			version = documentListGrid.cells(array[i], 4).getValue();				     // 버전
		   			state = documentListGrid.cells(array[i], 5).getValue();	 		 	         // 상태
		   			creator = documentListGrid.cells(array[i], 6).getValue();     			     // 등록자
		 			createDate = documentListGrid.cells(array[i], 7).getValue();			 	 // 등록일
		  			updateDate = documentListGrid.cells(array[i], 8).getValue();			 	 // 수정일
	  			}
	  			...
	  			returnArr[i][8] = manufacture;
			}
			opener.parent.addDoc(returnArr);
		}else if(type == "link") {
			var docOid = array[0];
...
----------------------------------------------------------%>
function  lfn_Search(){
	var form = $("form[name=selectOtherForm]").serialize();
	var url	= getURLString("doc", "listDocumentAction", "do") + "?select=true";
	var type= $("#searchType").val();
	
	if(type == 'ROHS'){
		url = getURLString("rohs", "listRohsAction", "do") + "?select=true";
	}
...
<body>
<form name="selectOtherForm" method="post">
					...
					<td>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" style="border-left-width: 1px;border-left-style: solid;border-left-color: #e6e6e6;">
							...
							<tr bgcolor="ffffff" height="35">
								<c:choose>
									<c:when test="${searchType eq 'MOLD'}">
										<td class="tdblueM">${f:getMessage('금형문서번호')}</td>
										<td class="tdwhiteL">
								...
								<c:choose>
									<c:when test="${searchType eq 'MOLD'}">
										<td class="tdblueM">${f:getMessage('금형문서명')}</td>
										<td class="tdwhiteL">
							...
							<tr bgcolor="ffffff" height="35">
								...
								<td class="tdwhiteL">
									...
									<a href="JavaScript:searchUser('selectOtherForm','true','creator','creatorName','people')">
				...
				<tr>
					<td>
						<table border="0" cellpadding="0" cellspacing="4" align="right">
				              <tr>
				              	 <td>
					                  <button type="button" name="" value="" class="btnCRUD" title="${f:getMessage('선택')}" id="selectBtn" name="selectBtn">
					                  	<span></span>
					                  	${f:getMessage('선택')}
				                  	  </button>
							      </td>
