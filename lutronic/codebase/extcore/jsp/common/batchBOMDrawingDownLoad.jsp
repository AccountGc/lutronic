...
$(function() {
	$('#attachDown').click(function() {
		...
		var form = $("form[name=batchBOMDrawingDown]").serialize();
		var url	= getURLString("common", "batchBOMDrawingDownAction", "do");
		$.ajax({
			type:"POST",
			url: url,
			data:form,
...
<form name=batchBOMDrawingDown method=post>
<input type="hidden" name="oid" id="oid" value="<c:out value="${oid }" />" />
<input type="hidden" name="ecoOid" id="ecoOid" value="<c:out value="${ecoOid }" />" />
