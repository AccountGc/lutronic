...
window.documentType = function(documentType, searchType) {
	...
	addSelectList('documentType', eval(data.responseText), searchType);
}
...
function lfn_DhtmlxGridInit() {
	...
	var COLNAMEARR = {
			...
			,col03 : '${f:getMessage('문서번호')}'
			,col04 : '${f:getMessage('내부 문서번호')}'
			,col05 : '${f:getMessage('프로젝트코드')}'
			,col06 : '${f:getMessage('문서명')}'
			,col07 : '${f:getMessage('문서분류')}'
			,col08 : '${f:getMessage('Rev.')}'
			,col09 : '${f:getMessage('상태')}'
			,col10 : '${f:getMessage('작성자')}'
			,col11 : '${f:getMessage('등록자')}'
			,col12 : '${f:getMessage('등록일')}'
			,col13 : '${f:getMessage('수정일')}'
	}
		...
		if(type == "select") {
			...
			for(var i =0; i < array.length; i++) {
				...
				var oid = array[i];															 // OID
	   			...
	   			var name = documentListGrid.cells(array[i], 5).getValue();				 // 품명
	   			var version = documentListGrid.cells(array[i], 7).getValue();				 // 버전
	   			var state = documentListGrid.cells(array[i], 8).getValue();	 		 // 상태
	   			var creator = documentListGrid.cells(array[i], 10).getValue();     // 등록자
	 			var createDate = documentListGrid.cells(array[i], 11).getValue();			 	 // 등록일
	  			var updateDate = documentListGrid.cells(array[i], 12).getValue();			 	 // 수정일
...
----------------------------------------------------------%>
function  lfn_Search(){
	var form = $("form[name=selectDocForm]").serialize();
...
<body>
<form name="selectDocForm" method="post">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" style="border-left-width: 1px;border-left-style: solid;border-left-color: #e6e6e6;">
							...
							<tr bgcolor="ffffff" height="35">
								...
								<td class="tdwhiteL">
									...
									<a href="JavaScript:searchUser('selectDocForm','true','creator','creatorName','people')">
