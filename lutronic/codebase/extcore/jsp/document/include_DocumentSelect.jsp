...
$(function() {
	...
	*                      관련 문서 추가
	...
	$("#addDoc").click(function() {
		...
		var url = getURLString("doc", "selectDocPopup", "do") + "?state="+state +"&searchType="+$("#searchType").val() + "&lifecycle="+$("#lifecycle").val();
		openOtherName(url,"selecDocPopup","1300","600","status=no,scrollbars=yes,resizable=yes");
	}),
	<%----------------------------------------------------------
	*                      관련 문서 삭제
	----------------------------------------------------------%>
	...
	$(":button[name='docCreate']").click(function() {
		var outputOid = this.value;
		var url = getURLString("doc", "createDocumentPop", "do") + "?type=control";
...
function addConDoc(obj) {
	...
	for (var i = 0; i < obj.length; i++) {
		...
		html += "	<td class='tdwhiteM'>";
		html += "		<input type='checkbox' name='docDelete' id='docDelete'>";
		html += "		<input type='hidden' name='" + docOid +"' id='docOid' value='" + obj[i].oid + "' />";
		html += "	</td>";
		html += "	<td class='tdwhiteM'>";
		html += obj[i].number;
		html += "	</td>";
								...
								<td align="left">
									<table border="0" cellpadding="0" cellspacing="2">
										<tr>
											...
											<c:if test="${type eq 'direct'}">
												<td>
													<button type="button" class="btnCustom" name="docCreate" id="docCreate" value="${oid }">
														<span></span>
														${f:getMessage('직접등록')}
													</button>
												</td>
									...
									<table width="100%" cellspacing="0" cellpadding="1" border="0" id="docTable" align="center">
										<tbody>
											<tr>
												...
												<td class="tdblueM"  width="35%">${f:getMessage('문서번호')}</td>
