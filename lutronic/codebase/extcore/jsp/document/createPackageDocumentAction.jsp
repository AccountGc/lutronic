...
$(document).ready(function(){
	...
	documentListGrid.enableAutoHeight(true, parseInt($(window).height())-200, true);
	
	  $(window).resize(function() { // 브라우저 사이즈 감지
		  documentListGrid.enableAutoHeight(true, parseInt($(window).height())-200, true);
		  documentListGrid.enableAutoWidth(true, 500, 200);
	  })
})
<%----------------------------------------------------------
*                      그리드 초기 설정
...
function lfn_DhtmlxGridInit(){
	var COLNAMEARR = {
			 col01 : 'NO'
			,col02 : '${f:getMessage('문서종류')}'
		 	,col03 : '${f:getMessage('문서명')}'
		 	,col04 : '${f:getMessage('결재방식')}'
			,col05 : '${f:getMessage('문서분류')}'
			,col06 : '${f:getMessage('문서코드')}'
			,col07 : '${f:getMessage('프로젝트명')}'
			,col08 : '${f:getMessage('프로젝트코드')}'
			,col09 : '${f:getMessage('내부 문서번호')}'
			,col10 : '${f:getMessage('부서명')}'
			,col11 : '${f:getMessage('부서 코드')}'
			,col12 : '${f:getMessage('보존기간')}'
			,col13 : '${f:getMessage('보존기간 코드')}'
			,col14 : '${f:getMessage('문서분류')}'
			,col15 : '${f:getMessage('파일명')}'
			,col16 : '${f:getMessage('부품번호')}'
			,col17 : '${f:getMessage('성공여부')}'
			,col18 : '${f:getMessage('에러내용')}'
	}
	...
	documentListGrid.enableAutoHeight(true, '100', true);
