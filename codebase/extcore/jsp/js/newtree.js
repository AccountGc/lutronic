...
function Tree(tid, rootName, dir, rootDesc)
{
    ...
    */
	this.iconDir = null;
	if ( dir == null ) {
		this.iconDir = "/Windchill/e3pscore/portal/images/tree/project";
	} else {
		this.iconDir = dir;
	}
    ...
    */
	this.leftSideForIE = function(node) {
		...
		if (node == this.root ) {
			...
			if(this.openedNode == node) {
				...
				if(node.icon != "") {
					...
					//alert(folderIcon.src);
				}
				
				renderString = renderString + "<SPAN style='color:" + fg + ";letter-spacing:-1px; background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'>"+"<IMG border='0' src='" + folderIcon.src + "' align='absmiddle'>" + node.name + "</SPAN>";	
			} else {
				var folderIcon = this.CLOSED_DEFAULT_FOLDER_ICON;
				if(node.icon != "") {
					folderIcon = new Image();
		...
		renderString = renderString + "<SPAN style='color:" + fg + ";letter-spacing:-1px; background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])' onmouseover='popupShow(\"" + nameValue.replace("\'","\\\'").replace("\"","\\\"") + "\")' onMouseOut=\"javascript:popupHide()\" >" + node.name + "</SPAN> ";
		//renderString = renderString + "<SPAN onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])' >" + node.name + "</SPAN> ";
		
		//renderString = renderString + node.name;
