...
//	}
	...
	@Override
	public ContentHolder attach(ContentHolder holder, String primary, String[] secondary, String[] orgSecondary)
			throws Exception {
		return attach(holder, primary, secondary, orgSecondary, true);
	}

	@Override
	public ContentHolder attach(ContentHolder holder, String primary, String[] secondary, String[] delSecondary,
	...
	public ContentHolder attach(ContentHolder holder, String primary, String[] secondary, String[] delSecondary,
		...
		if (isWorkingCopy) {
			...
			if (ofiles != null) {
				for (int i = 0; i < ofiles.size(); i++) {
					...
					// oad.getFileName());
					boolean flag = false;
					// System.out.println("null!=delSecondary =?"
					...
					// System.out.println("----------------------- 대상 파일 flag >>>>>>>>>>>>>>>>> " +
					// flag);
					if (!flag) {
						// System.out.println("----------------------- 대상 파일
						// oad.getRole().toString().equals() >>>>>>>>>>>>>>>>> " +
						// (oad.getRole().toString().equals("SECONDARY")));
						// System.out.println("----------------------- description == null &&
						// oad.getDescription() == null >>>>>>>>>>>>>>>>> " + (description == null &&
						// oad.getDescription() == null));
						// System.out.println("----------------------- description != null &&
						// description.equals(oad.getDescription())l >>>>>>>>>>>>>>>>> " + (description
						// != null && description.equals(oad.getDescription())));
						// System.out.println("----------------------- description
						// !oad.getRole().toString()="+oad.getRole().toString());
		...
		// System.out.println("----------------------- 대상 파일 secondary >>>>>>>>>>>>>>>>>
		// " + (secondary != null));
		if (secondary != null) {
	 ...
	 */
	@Override
	public List<BatchDownData> getAttachFileList(ContentHolder holder, List<BatchDownData> list, String describe)
			throws Exception {
		...
		if (applist.size() == 0) {
			...
			for (ApplicationData appData : applist) {
				...
				list.add(data);
				// System.out.println(appData.getFileName() +","
				// +appData.getRole().getDisplay()+"," );
			}
		}
