	...
	public static int distributeInnerType(WTUser user) {
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				/*
				 * if(group.getBusinessType().equals("WTOrganization")){ continue; }
				 */
				if (!group.getDomainRef().equals(user.getDomainRef())) {
					continue;
				}
				// System.out.println(group.getName() +
				// ","+group.getBusinessType()+","+group.getType()
				// +","+group.getDomainRef().getName() +","+group.getContainerPath());

				/*
				 * if(group.getName().equals("DistributeInner")){ isDistribute = true; }else{
				 * isInner = true; }
				 */
				if (group.getBusinessType().equals("WTGroup")) {

					if (group.getName().equals("DistributeInner")) {
						// System.out.println("CCC = "+group.getName() +
						// ","+group.getBusinessType()+","+group.getType()
						// +","+group.getDomainRef().getName() +","+group.getContainerPath());
						isDistribute = CommonUtil.isMember("DistributeInner", user);
					} else {
						// System.out.println("EEE = "+group.getName() +
						// ","+group.getBusinessType()+","+group.getType()
						// +","+group.getDomainRef().getName() +","+group.getContainerPath());
				 ...
				 * 
				 * System.out.println("st =" + st); if (st.equals ( DistributeGropuInner )) {
				 * 
				 * }else{ w }
				 */
