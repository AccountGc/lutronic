...
public class BomBroker {
	...
	public void compareBom(PartTreeData data, PartTreeData data2,
			ArrayList result,boolean isALL) { 

		result.add(new PartTreeData[] { data, data2 });
		
		//System.out.println("org data =" + data.children.size());
		//System.out.println("baseline data =" + data2.children.size());
	...
	public void setHtmlForm(PartTreeData data1, PartTreeData data2,
			ArrayList result) {
	...
	public PartTreeData getTree(WTPart part, boolean desc, Baseline baseline, View view)
		throws Exception {
		PartTreeData root =  getTree(part, null, 0, desc, baseline, view);
		sort(root);
		return root;
	}
	
	public PartTreeData getTree(WTPart part, boolean desc, Baseline baseline)
			throws Exception {
	...
	public PartTreeData getTree(WTPart part, WTPartUsageLink link, int level,
			boolean desc, Baseline baseline) throws Exception {
		return getTree(part,link,level,desc,baseline,null);
	}

	public PartTreeData getTree(WTPart part, WTPartUsageLink link, int level,
			boolean desc, Baseline baseline, View view) throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			//System.out.println(" Tree cPart =  " + ((WTPart) o[1]).getNumber());
			PartTreeData cd = getTree((WTPart) o[1], (WTPartUsageLink) o[0],
					level + 1, desc, baseline, view);
	...
	public PartTreeData getOneleveTree(WTPart part,Baseline baseline) throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			//System.out.println( baseline+":"+cd.number +":"+link.getQuantity().getAmount());
			data.children.add(cd);
		}
		return data;
...
public PartTreeData getOneleveTreeAsc(WTPart part,Baseline baseline) throws Exception {
	...
	public ArrayList descentLastPart(WTPart part, Baseline baseline, State state)
			throws WTException {
	...
	public ArrayList descentLastPart(WTPart part, View view, State state)
			throws WTException {
		ArrayList v = new ArrayList();
		if (!PersistenceHelper.isPersistent(part))
			return v;
	...
	public ArrayList ancestorPart(WTPart part, View view, State state)
			throws WTException {
		ArrayList v = new ArrayList();
		try {
	...
	public ArrayList ancestorPart(WTPart part, Baseline baseline, State state)
			throws WTException {
		ArrayList v = new ArrayList();
		try {
			WTPartMaster master = (WTPartMaster) part.getMaster();
