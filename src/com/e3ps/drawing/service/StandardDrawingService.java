...
//	static final String BLANK_IMG = WebUtil.getHost()+"netmarkets/images/blank24x16.gif";
	...
	public String getPrefix(String fileName) {
		...
		// ="+changeName.substring(changeName.length()-3));
	...
	public boolean isFileNameCheck(String fileName) {
		...
		try {
			...
			qs.appendWhere(new SearchCondition(EPMDocumentMaster.class, EPMDocumentMaster.CADNAME,
					SearchCondition.EQUAL, fileName, false), new int[] { 0 });
	...
	public EPMDocumentType getEPMDocumentType(String fileName) {
		/*
		 * CADASSEMBLY.value=����� CADCOMPONENT.value=CAD ��ǰ CADDRAWING.value=�����
		 * DIAGRAM.value=���̾�׷� ECAD-ASSEMBLY.value=ECAD - ����� ECAD-BOARD.value=ECAD
		 * - ���� ECAD-COMPONENT.value=ECAD - ������Ʈ ECAD-CONTENT.value=ECAD - ����Ʈ
		 * ECAD-SCHEMATIC.value=ECAD - ���� ECAD-SOURCE.value=ECAD - �ҽ�
		 * FORMAT.value=���� LAYOUT.value=���̾ƿ� MANUFACTURING.value=����
		 * MARKUP.value=��ũ�� OTHER.value=��Ÿ REPORT.value=���� SKETCH.value=����ġ
		 * UDF.value=����� ���� ��� DESIGN.value=���� RENDERING.value=������
		 * PUB_COMPOUNDTEXT.value=�Խ� �ҽ� PUB_GRAPHIC.value=�Խ� �׷���
		 * PUB_CADVIEWABLE.value=���� ������ CAD �׸� �Խ� CADDRAWINGTEMPL.value=�����
		 * ���ø� ANALYSIS.value=�м� IGES.value=Iges STEP.value=Step VDA.value=VDA-FS
		 * ACIS.value=ACIS PARASOLID.value=Parasolid ZIP.value=Zip ���� DXF.value=DXF
		 * NOTE.value=�޸� WORKSHEET.value=Mathcad ��ũ��Ʈ EDA_DIFF_CONFIG.value=ECAD
		 * Compare ���� ������ EDA_DIFF_REPORT.value=ECAD Compare IDX ������
		 * CALCULATION_DATA.value=��� ������ MANIKIN_POSTURE.value=��ü ���� �ڼ�
		 * WORKPLANE.value=�۾� ��� WORKPLANE_SET.value=�۾� ��� ��Ʈ
		 * CUTTER_LOCATION.value=Ŀ�� ��ġ MACHINE_CONTROL.value=�ӽ� ���� ������
		 * INSTANCEDATA.value=�ν��Ͻ� ������ MECHANICARESULTS.value=�м� �� ���� ���� ���
	...
	public QuerySpec getListQuery(HttpServletRequest request) throws Exception {
		...
		try {
			...
			if (sortValue != null && sortValue.length() > 0) {
				...
				query.appendOrderBy(
						new OrderBy(new ClassAttribute(EPMDocument.class, EPMDocument.MODIFY_TIMESTAMP), true),
						new int[] { idx });
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Hashtable modifyEpm(EPMDocument orgEpm, Hashtable hash, String[] loc, String[] deloc, String[] partOid)
			throws Exception {
		Hashtable rtnVal = new Hashtable();
		...
		if (orgEpm != null) {
			...
			/*
			 * CommonContentHelper.service.delete(newEpm); if(deloc != null){ for(int j=0;
			 * j< deloc.length; j++){ ApplicationData ad = (ApplicationData)
			 * rf.getReference(deloc[j]).getObject();
			 * CommonContentHelper.service.attach(newEpm, ad, false); } }
			 * 
			 * if(loc != null){ for(int i=0; i< loc.length; i++){
	...
	public EPMDocument getLastEPMDocument(String number) {
		...
		try {
			...
			qs.appendAnd();
			qs.appendWhere(new SearchCondition(class1, EPMDocument.NUMBER, SearchCondition.EQUAL, number),
					new int[] { i });
	...
	public String updateIBA(EPMDocument epm, WTPart part) throws WTException {
		...
		try {
			...
			/*
			 * IBAUtil.changeIBAValue(epm, AttributeKey.EPMKey.IBA_MATERIAL, Material,
			 * "string");
			 */
	...
	public ResultData createDrawing(Map<String, Object> map) {
		...
		try {
			...
			// System.out.println("--------------------EPM Document Create
			// Success------------------------");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Map<String, String> create(Map<String, Object> hash, String[] loc) {
		...
		try {
			...
			/*
			 * String authoringType = EpmUtil.getExtension(primary); authoringType =
			 * authoringType.toUpperCase();
			 * 
			 * if (authoringType.equals("DSN") || authoringType.equals("BRD") ||
			 * authoringType.equals("ZIP")){ authoringType = "ORCAD"; }else
			 * if(authoringType.equals("DWG")){ authoringType = "ACAD"; }else{ authoringType
			 * = "OTHER"; }
			 */
			...
			if ("1".equals(createType) || !"".equals(partToDrwOid)) {
				...
				String subcontract = StringUtil
						.checkNull(IBAUtil.getAttrValue(part, AttributeKey.EPMKey.IBA_SUBCONTRACT));
			...
			/*
			 * if (!"".equals(newFileName)) { CommonContentHelper.service.attachPrimary(epm,
			 * newFileName); }
			 * 
			 * if(loc != null){ for(int i=0; i< loc.length; i++){
	...
	public String createPackageDrawingAction(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			xmlBuf.append(
					"<cell colspan='5'><![CDATA[<font color='red'>" + e.getLocalizedMessage() + "</font>]]></cell>");
	...
	private void addPrimaryList(Vector<ApplicationData> vecApp, EPMDocument epm)
	...
	private void addSecondList(Vector<ApplicationData> vecApp, EPMDocument epm, boolean isCheck)
			throws WTException, PropertyVetoException {
		ContentHolder holder = ContentHelper.service.getContents((ContentHolder) epm);
		...
		if (isCheck) {
			/*
			 * while(qr.hasMoreElements()) { ContentItem item = (ContentItem) qr.nextElement
			 * (); if(item != null) { ApplicationData appData = (ApplicationData)item;
			 * if("{$CAD_NAME}".equals(appData.getFileName())){ EPMDocument epms =
			 * (EPMDocument)holder; appData.setFileName(epms.getCADName());
			 * System.out.println("addSecondList ::: isCheck = "
			 * +isCheck+"\tadd appData Name =" +appData.getFileName()); }
			 * if(appData.getFileName().lastIndexOf("dxf")>0){
			 * System.out.println("addSecondList ::: isCheck = "
			 * +isCheck+"\tadd After appData Name =" +appData.getFileName()); }
			 * //vecApp.add(appData); } }
			 */
				...
				for (int l = 0; l < contentList.size(); l++) {
					...
					if (contentitem instanceof ApplicationData) {
						...
						if (drawAppData.getRole().toString().equals("SECONDARY")
								&& drawAppData.getFileName().lastIndexOf("dxf") > 0) {
							// System.out.println("addSecondList ::: isCheck = "+isCheck+"\tadd appData Name
