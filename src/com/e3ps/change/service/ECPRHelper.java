...
//			String[] changeSections = (String[])params.get("changeSections");
			...
			*/
			...
			if(model.length() > 0) {
				...
				qs.appendWhere(new SearchCondition(ecrClass, EChangeRequest.MODEL, SearchCondition.EQUAL, model, false), new int[] {ecoIdx});
			}
			...
			qs.appendWhere(new SearchCondition(ecrClass, EChangeRequest.EO_TYPE, SearchCondition.EQUAL, "ECPR", false), new int[] {ecoIdx});
	...
	public JSONArray include_ECRView(String oid) throws Exception{
		...
		try {
			...
			if(obj instanceof EChangeOrder){
				...
				List<EcrToEcrLink> list_ECRtoECRLINK = ECPRHelper.service.getEcrToEcrLinks(ecr, "useBy");
				for(EcrToEcrLink link : list_ECRtoECRLINK){
					EChangeRequest linkECR = link.getUseBy();
					ECRData data = new ECRData(linkECR);
					list.add(data);
				}
				list_ECRtoECRLINK = ECPRHelper.service.getEcrToEcrLinks(ecr, "used");
				for(EcrToEcrLink link : list_ECRtoECRLINK){
