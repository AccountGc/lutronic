...
//			String[] changeSections = (String[])params.get("changeSections");
			...
			*/
			...
			qs.appendWhere(new SearchCondition(ecrClass, EChangeRequest.EO_TYPE, SearchCondition.EQUAL, "CR", false), new int[] {ecoIdx});
	...
	public JSONArray include_ECRView(String oid) throws Exception{
		...
		try {
			...
			if(obj instanceof EChangeOrder){
				...
				List<EcrToEcrLink> list_ECRtoECRLINK = CRHelper.service.getEcrToEcrLinks(ecr, "useBy");
				for(EcrToEcrLink link : list_ECRtoECRLINK){
					EChangeRequest linkECR = link.getUseBy();
					ECRData data = new ECRData(linkECR);
					list.add(data);
				}
				list_ECRtoECRLINK = CRHelper.service.getEcrToEcrLinks(ecr, "used");
				for(EcrToEcrLink link : list_ECRtoECRLINK){
